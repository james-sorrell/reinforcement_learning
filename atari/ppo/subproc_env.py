...
class EnvActor(object):
    ...
    def step_env(self, t):
        if t == start_t:
            ...
            value_0, _ = self.model.network(np.expand_dims(self.last_obs, axis=0))
            self.val.append(value_0[0])
        
        _, policy_t = self.model.network(np.expand_dims(self.last_obs, axis=0))
        #policy_t = sess.run(policy, {obs_ph: [self.last_obs]})[0]
        
        policy_t = np.array(policy_t)
        policy_t /= policy_t.sum()
        action_t = np.random.choice(self.num_actions, 1, p=policy_t)[0]
        ...
        val_tp1, _ = self.model.network(np.expand_dims(obs_tp1, axis=0))
    ...
    def calculate_horizon_advantages(self, end_t):
        ...
        value_estimates = []
        advantage_so_far = 0
        # No empirical estimate beyond end of horizon, so use value function.  
        # Is immediately reset to 0 if at episode boundary.
