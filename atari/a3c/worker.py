...
class Worker:
  ...
  def run(self, coord, t_max):
    ...
    try:
      while not coord.should_stop():
        ...
        self.policy_net.network.set_weights(self.global_policy_net.network.get_weights())
        self.value_net.network.set_weights(self.global_value_net.network.get_weights())
  ...
  def update(self, steps):
    ...
    reward = 0.0
    if not steps[-1].done:
      reward = self.get_value_prediction(steps[-1].next_state)
    ...
    self.global_policy_net.train(self.policy_net.compute_gradients(np.array(states), advantages, actions))
    self.global_value_net.train(self.value_net.compute_gradients(np.array(states), value_targets))
